...
const HC1Card= ({name, title, url, imageUrl, isScrollable, bg_color}) => (
    <HC1CardWrapper href={url} style={{backgroundColor: {bg_color}}}>
        <div className={isScrollable ? "HC1CardNoArrow" : "HC1CardNoArrow nonScrollable"}>
            <img src={imageUrl} />
            <div>
              <b>{title}</b>
              <p>{name}</p>
    ...
    <HC1CardsWrapper>
        <div className="NoArrowScrollable HC1Cards">
          {scrollable === undefined
            ? null
            : scrollable.map(({title, formatted_title: { text: formattedTitle }, name, url, icon: { image_url: imageUrl },
            }) => (
                  <HC1Card
                    key={name}
                    name={name}
        ...
        <div className="HC1Cards">
          {nonScrollable === undefined
            ? null
            : nonScrollable.map(( {title, formatted_title: { text: formattedTitle }, name, url, icon: { image_url: imageUrl },
            }) =>  (
                  <HC1Card
                    key={name}
                    name={name}
